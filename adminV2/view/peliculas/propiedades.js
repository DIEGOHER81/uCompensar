...
$(document).ready(function () {
    ...
    if (jQuery("#idPropiedad").val() != "") {
        ConsultarCaracteristicasDiligenciadas(jQuery("#idPropiedad").val());
    ...
    var calendar = $('#calendar').fullCalendar({  // assign calendar
        ...
        events: consultarEventos(jQuery("#idPropiedad").val()), // request to load current events
    ...
    function consultarEventos(idpropiedad) {
        var parametros = {
            "Opc": 'MostrarFechasPropiedad',
            "id" : idpropiedad
        };
        jQuery.ajax({
            type: 'post',
            ...
            success: function (datos) {
                ...
                ConsultarEventosTabla(idpropiedad);
            }
        });
    }

    function ConsultarEventosTabla(idpropiedad)
    {
        var parametros = {
            "Opc":'MostrarFechasPropiedadestbl',
            "id":idpropiedad
        };
        jQuery.ajax({
           type:'post',
           ...
           async: true,
           success:function(datos){
              jQuery("#tbllstFechas").html(datos)
...
/*
    ...
    {
        var parametros = {
            "Opc":'QueryHighSeassonDates'
        };
        jQuery.ajax({
           type:'post',
           url:'../../controller/highSeason.controller.php',
           data:parametros,
           async: true,
           success:function(datos){
               var dataJson = eval(jQuery.trim(datos))
                 ...
                 $("#calendar").fullCalendar('renderEvent',
                    {
                        ...
                        description:'Temporada alta'
                    },
                    true);
               }
               ConsultarEventosTabla();
           }
        });
    }
 
    function ConsultarEventosTabla()
    {
        var parametros = {
            "Opc":'MostrarFechasPropiedadestbl'
        };
...
*/
    ...
    $("#btnsave").click(function () {
            ...
            // Deshabilitar el botón de guardar para evitar clics múltiples
            // this.disabled = true;

            // Verificar si hay archivos en cola en alguno de los dropzones
            if (myDropzone_lista.getQueuedFiles().length === 0 && myDropzone_DocumentosLegales.getQueuedFiles().length === 0) {
                // Si no hay archivos en cola en ninguno de los dropzones, llamar a SavePropiedad
                SavePropiedad();
            }
            
            // Variables para verificar si cada dropzone ha completado su proceso
            var listaCompleted = false;
            var documentosCompleted = false;

            if (myDropzone_lista.getQueuedFiles().length !== 0) {
                console.log("Entre a procesar primer dropzone")
                myDropzone_lista.processQueue();
            } else {
                listaCompleted = true;
            }

            if (myDropzone_DocumentosLegales.getQueuedFiles().length !== 0) {
                console.log("Entre a procesar segundo dropzone")
                myDropzone_DocumentosLegales.processQueue();
            } else {
                var documentosCompleted = true;
            }


            

            // Esperar a que myDropzone_lista complete su proceso
            console.log("Primer Dropzone: " + myDropzone_lista.getQueuedFiles().length);
            myDropzone_lista.on("queuecomplete", function() {
                console.log("Entre a listacompleted")
                listaCompleted = true;
                // Verificar si ambos dropzones han completado su proceso y llamar a la función de verificación y guardado
                checkCompletionAndSave();
            });

            // Esperar a que myDropzone_DocumentosLegales complete su proceso
            console.log("Segundo Dropzone: " + myDropzone_DocumentosLegales.getQueuedFiles().length);
            myDropzone_DocumentosLegales.on("queuecomplete", function() {
                console.log("Entre a documents completed")
                documentosCompleted = true;
                // Verificar si ambos dropzones han completado su proceso y llamar a la función de verificación y guardado
                checkCompletionAndSave();
            });

            // Función para verificar si ambos dropzones han completado su proceso
            function checkCompletionAndSave() {
                if (listaCompleted && documentosCompleted) {
                    // Verificar si hay archivos en cola en alguno de los dropzones
                    if (myDropzone_lista.getQueuedFiles().length === 0 && myDropzone_DocumentosLegales.getQueuedFiles().length === 0) {
                        // Si no hay archivos en cola en ninguno de los dropzones, llamar a SavePropiedad
                        SavePropiedad();
...
async function ConsultarInformacionPropiedad(idPropiedad) {
    ...
    try {
        ...
        ConsultarCaracteristicasRelacionadaslst(dataJson[i].codtipo);
...
function ConsultarCaracteristicasDiligenciadas(idPropiedad)
{
    var parametros = {
        "Opc": "CaracteristicasDiligenciadas",
        "id": idPropiedad 
    };
